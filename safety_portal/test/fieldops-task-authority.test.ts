...
// Roster person, optionally linked to an account (personnel.username == users.username).
async function seedPersonnel(name: string, username: string | null): Promise<number> {
  await env.DB.prepare("INSERT INTO personnel (name, username, active) VALUES (?,?,1)").bind(name, username).run();
