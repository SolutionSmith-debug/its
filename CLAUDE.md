...
| `operator_dashboard/` | Working, ships **DARK** (fail-closed until `ITS_OPERATOR_PIN` set). WS2 D1-1/D1-2/D1-3 | Localhost-only FastAPI (`python -m operator_dashboard` @127.0.0.1:8484, Tailscale-exposed). Read-only obs panels (`sources/`: launchd / watchdog markers / breaker / heartbeats / locks / log-tail / errors / review-queue) + PIN-gated ACT surface (`act/`: Class-A `ITS_Config` editor, Class-B daemon interval/control · breaker-clear · **error-log clear** [prunes terminal `ITS_Errors` rows, NEVER an open CRITICAL — reuses watchdog Check O's `shared/errors_rotation` predicate, the single source of truth], Class-C write-only secret rotation + PIN change); `auth.py` PIN + elevated-confirm, constant-time, fail-closed. Writes `ITS_Config` + prunes `ITS_Errors` terminal rows (internal SoR) — **never deploys / sends externally** (§50 enqueue is the SPA's job). No launchd plist yet (D1-3b). Blueprint `workstreams/operator-dashboard/mission.md`. |
