...
3. **Doctrine (canonical — planning-layer wins)** — `~/its-blueprint/doctrine/` (Operational Standards v20,
...
1. **Planning & Foundation** (Claude.ai project, not in this repo). Mission files, architectural
   ...
   v11, Operational Standards v20, Vision & Roadmap v9, Handover Plan v9.

   _Operational Standards is canonically at **v20** (`../its-blueprint/doctrine/operational-standards.md`,
   `status: canonical`); **v20 is the governing version — every `Op Stds §N` citation in this file
   ...
   v19.x riders). Still-load-bearing
...
1. **Sender allowlist + scope enforcement + header-forgery detection.** The polling-daemon
   pattern (canonical per Op Stds v20 §31; first exercised by the now-retired
   `safety_reports/intake_poll.py`, carried forward by Email Triage) fetches from allowlisted
   senders via Graph; non-allowlisted email routes to Quarantine. ITS_Trusted_Contacts sheet (Op Stds v20 §33) is the canonical allowlist
...
6. **Attachment screening pipeline.** Every attachment passes through four sub-layers per
   Op Stds v20 §34: (a) static signatures (magic-number, size, filename); (b) format-aware
...
## Maintenance & successor-operator model (FM v11 · Op Stds v20 §§43–44)

ITS is built to be maintained after the developer (Seth) departs. The model (FM v11; Op Stds v20
§44) has **three tiers**:
...
**Training-enforced, NOT structurally enforced** (the Op Stds v20 §44 / FM v11 reframe). No
...
- **Kill switch first.** Call `shared.kill_switch.check_system_state()` (or use `@require_active`)
  ...
  kill switch, is the security boundary (Op Stds v20 §1).
...
| `operator_dashboard/` | Working, ships **DARK** (fail-closed until `ITS_OPERATOR_PIN` set). WS2 D1-1/D1-2/D1-3 | Localhost-only FastAPI (`python -m operator_dashboard` @127.0.0.1:8484, Tailscale-exposed). Read-only obs panels (`sources/`: launchd / watchdog markers / breaker / heartbeats / locks / log-tail / errors / review-queue) + PIN-gated ACT surface (`act/`: Class-A `ITS_Config` editor, Class-C write-only secret rotation); `auth.py` PIN + elevated-confirm, constant-time, fail-closed. Writes ONLY `ITS_Config` (internal SoR write) — **never deploys / sends** (§50 enqueue is the SPA's job). No launchd plist yet (D1-3b). Blueprint `workstreams/operator-dashboard/mission.md`. |
...
8. launchd plists live in `scripts/launchd/` as templates; `install.sh` copies them to
   ...
   intake-bearing workstreams** (Op Stds v20 §31; `safety_reports/portal_poll.py` is the
...
update-in-place per cycle. Push surface per Op Stds v20 §3.1 + §32.
...
Operational Standards v20), the planning project wins. Flag the inconsistency.
