...
# fieldops-sync, po-poll, po-send, subcontract-poll, subcontract-send, estimate-poll): these plists carry the placeholder in <integer>StartInterval</integer>.
# `load`/`dry-run` resolve it from the optional [interval] arg, else a per-daemon default
# (900 / 60 / 90 / 900 / 90 / 90 / 900 / 120 / 900 / 120 respectively, matching the daemon's ITS_Config poll-interval row
...
# po_materials.estimate_poll .poll_interval_seconds). The interval is BAKED into
