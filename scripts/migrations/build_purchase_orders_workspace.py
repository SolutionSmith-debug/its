"""Build the "ITS — Purchase Orders" workspace + its "Control" folder.

S1 of the PO workstream (Aug-7 delivery program, `docs/2026-07-09_aug7_delivery_program.md`).
The EIGHTH standalone workspace — like the Safety Portal and Progress Reporting workspaces
it sits OUTSIDE the §23 audience-separation model and is governed by §46: **workspace
membership = approval authority**. The share list of THIS workspace is the approver set the
F22 gate verifies before `po_send` dispatches a PO to a vendor (decision D11 — Evergreen
limits who may approve by limiting who is shared here).
...
Creates (find-or-create, idempotent):
  1. The "ITS — Purchase Orders" WORKSPACE, if absent.
  2. The "Control" FOLDER inside it — home of the three cross-job PO sheets
     (ITS_Vendors + PO_Log + PO_Pending_Review). Per-job artifacts (the PO PDFs)
     live in Box, not here.

Cutover sequence (FLIP precedes SEED):
  1. THIS script — note the printed WORKSPACE + FOLDER ids.
  2. Flip WORKSPACE_PURCHASE_ORDERS + FOLDER_PO_CONTROL in shared/sheet_ids.py.
  3. build_its_vendors_sheet.py + build_po_log_sheet.py + build_po_pending_review_sheet.py
     (each find-or-creates the SAME "Control" folder by name — order-independent with
     this script and with each other).
  4. Flip SHEET_ITS_VENDORS + SHEET_PO_LOG + SHEET_PO_PENDING_REVIEW.
  5. seed_its_vendors.py (refuses to run while SHEET_ITS_VENDORS is 0).
  6. OPERATOR (send-blocking): share every PO approver into "ITS — Purchase Orders"
     (§46 — an approver not shared here cannot approve a PO_Pending_Review row; an
     empty resolved set fails closed and blocks all PO sends).
...
Run from ~/its (or a worktree):
    python3 scripts/migrations/build_purchase_orders_workspace.py --dry-run
    python3 scripts/migrations/build_purchase_orders_workspace.py
...
WORKSPACE_NAME = "ITS — Purchase Orders"
FOLDER_NAME = "Control"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the ITS — Purchase Orders workspace + Control folder (PO S1)."
    )
    ...
    print("\nSummary:")
    print(f"  WORKSPACE_PURCHASE_ORDERS: id={workspace_id}")
    print(f"  FOLDER_PO_CONTROL:         id={folder_id}")
    print("\nNext: flip the two ids above, then run build_its_vendors_sheet.py + "
          "build_po_log_sheet.py + build_po_pending_review_sheet.py.")
    return 0
