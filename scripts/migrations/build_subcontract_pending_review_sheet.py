"""Build Subcontract_Pending_Review — the subcontract review/approve/send surface (S1).

Creates the sheet in the "Control" folder of the "ITS — Subcontracts" workspace.
One row per generated subcontract awaiting human review. `subcontract_poll` (S4) appends
the row when it files a rendered subcontract PDF; a human edits the Email Body, flips
"Approve for Scheduled Send" (or "Send Now"); the F22 gate verifies the cell-history
actor against the workspace share list (§46) before `subcontract_send` (S5) dispatches
...
what lets S5 bind a `subcontract_review` module without engine surgery. Three columns
carry subcontract semantics inside protocol-titled slots (descriptions say so on the
live sheet):

    "Job ID"       ← the **Sub Key** (SUB-###### — recipient join key → ITS_Subcontractors)
    "Week Of"      ← the **Subcontract Date**
    "Compiled PDF" ← the **Subcontract PDF** Box link

The twin is test-pinned against BOTH the WPR builder schema and the wsr_review COL_*
constants (tests/test_subcontract_s1_sheets.py) — retitling any protocol column breaks
the S5 bind and the tests red.

Workstream tag = "subcontracts" (P1b cross-workstream contamination guard): the send
guard HARD-HELDs any row whose tag ≠ the sending workstream, so a "safety"/"progress"
row here (or a "subcontracts" row on WSR/WPR) is a contamination signal, never a send.
...
Subcontract_Pending_Review already exists in it (order-independent with the other S1
builders).

Prereq: build_subcontracts_workspace.py has been run and WORKSPACE_SUBCONTRACTS
flipped in shared/sheet_ids.py.

    python3 scripts/migrations/build_subcontract_pending_review_sheet.py --dry-run
    python3 scripts/migrations/build_subcontract_pending_review_sheet.py
"""
...
FOLDER_NAME = "Control"
SHEET_NAME = "Subcontract_Pending_Review"
...
WORKSTREAM_OPTIONS = ["subcontracts"]  # P1b guard; this is the subcontract review sheet
...
COLUMN_SCHEMA: list[dict[str, Any]] = [
    ...
    {"title": "Compiled PDF", "type": "TEXT_NUMBER",
     "description": "PROTOCOL SLOT — the generated subcontract PDF's Box link. The PDF also attaches "
                    "to this row for one-click review."},
    {"title": "Email Body", "type": "TEXT_NUMBER",
     "description": "Editable body — THE source of truth subcontract_send transmits to the "
                    "subcontractor. The reviewer may edit before approving."},
    {"title": "Recipient TO", "type": "TEXT_NUMBER",
     "description": "Display of the resolved subcontractor contact email. Authoritative source is "
                    "ITS_Subcontractors (by Sub Key) at send time."},
    ...
    {"title": "Approve for Scheduled Send", "type": "CHECKBOX",
     "description": "Human approval gate. A person flips this; MODIFIED_BY auto-captures who; the "
                    "F22 gate verifies that actor is on the ITS — Subcontracts share list (§46) "
                    "before dispatch."},
    ...
    {"title": "Workstream", "type": "PICKLIST", "options": WORKSTREAM_OPTIONS,
     "description": "Report-family tag (P1b cross-workstream send guard). This is the subcontract "
                    "review sheet → 'subcontracts'; any other tag is contamination the send guard "
                    "HARD-HELDs."},
...
def build_sheet(*, dry_run: bool) -> tuple[str, int | None]:
    ...
    if existing_id is not None:
        ...
        print(f"[bootstrap] shared/sheet_ids.py:\n    SHEET_SUBCONTRACT_PENDING_REVIEW = {existing_id}")
    ...
    print(f"[bootstrap] Update shared/sheet_ids.py:\n    SHEET_SUBCONTRACT_PENDING_REVIEW = {new_id}")
    return "created", new_id


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build Subcontract_Pending_Review (subcontract S1 review surface).")
