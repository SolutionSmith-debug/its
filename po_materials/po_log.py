"""PO_Log access — the operator-visible ledger MIRROR of the D1 PO store (PO S4).

One row per generated PO. `po_poll`'s drafts pass APPENDS the row at filing time
(Status=pending_review — the D1 machine's state the moment mark-filed lands); the
status pass STAMPS the later transitions (approved / sent / superseded / canceled)
as it mirrors them to the Worker. D1 (via the Worker) remains the authoritative PO
status machine — this sheet is the ledger the operator reads, plus the transition
home of HAND-ISSUED POs (which is why `numbering.check_collision` scans it before
any filing).
...
collide) and (b) the supersession helpers (`find_po_number_by_d1_id` resolves a
...
* `append_filed_row` is APPEND-ONLY and idempotent-by-caller: the caller checks
  `find_row_by_po_number` first (the collision/retry logic in `numbering` + po_poll).
...
# ---- Column titles (mirror scripts/migrations/build_po_log_sheet.py) ----
COL_PO_NUMBER = "PO Number"          # primary — the contractual D7 identity
COL_JOB_PROJECT = "Job / Project"
COL_JOB_ID = "Job ID"
COL_VENDOR = "Vendor"                # vendor display name at generate time
COL_VENDOR_KEY = "Vendor Key"
COL_STATUS = "Status"                # PICKLIST — lowercase D1 vocabulary
COL_TOTAL = "Total"
COL_PO_PDF = "PO PDF"
COL_SUPERSEDES = "Supersedes"        # display: the predecessor's PO number
COL_SUPERSEDED_BY = "Superseded By"  # display: the successor's PO number
...
# STATUS_OPTIONS and picklist_validation._PO_LOG_STATUS_VALUES verbatim — 'queued'
# is deliberately absent: a queued PO has not been filed and has no ledger row yet).
STATUS_DRAFT = "draft"
...
LEGAL_STATUSES: frozenset[str] = frozenset({
    STATUS_DRAFT, STATUS_PENDING_REVIEW, STATUS_APPROVED,
    STATUS_SENT, STATUS_SUPERSEDED, STATUS_CANCELED,
})
...
def row_d1_id(row: dict[str, Any]) -> int | None:
    """Extract the Notes-encoded D1 id from a PO_Log row, or None (a hand-issued
    row keyed in by the operator carries no d1_id — that absence is exactly what
    makes it a COLLISION when its number matches a D1 allocation)."""
...
def find_row_by_po_number(
    po_number: str, *, sheet_id: int | None = None
) -> dict[str, Any] | None:
    """The ledger row whose PO Number == `po_number`, or None. The identity lookup
    behind the collision double-check + the status stamps.

    `sheet_id` defaults to the flat PO_Log; pass a per-job tracking sheet ID
    (shared/job_sheet.py) so the per-job append's idempotency guard runs against
    the TARGET sheet, independent of the flat ledger."""
    rows = smartsheet_client.get_rows(
        SHEET_ID if sheet_id is None else sheet_id,
        filters={COL_PO_NUMBER: po_number},
    )
    return rows[0] if rows else None


def find_po_number_by_d1_id(d1_id: int) -> str | None:
    """Resolve a D1 PO id → its ledger PO Number via the Notes-encoded d1_id.

    Used for supersession: the /pending payload names a predecessor only by
    `supersedes_po_id`, but the in-body clause + the Superseded-By display need the
    ...
    for row in smartsheet_client.get_rows(SHEET_ID):
        if row_d1_id(row) == d1_id:
            value = str(row.get(COL_PO_NUMBER) or "").strip()
...
def append_filed_row(
    *,
    ...
    `supersedes_display` is the resolved predecessor PO number ('' for a
    non-superseding PO). Caller guarantees no-collision/no-duplicate via
    `numbering.check_collision` first.

    `sheet_id` defaults to the flat PO_Log (the ledger SoR mirror); pass a per-job
    tracking sheet ID (shared/job_sheet.py — structure-cloned from this very sheet,
    so the column titles match) to mirror the row there. Callers guard duplicates
    per target via `find_row_by_po_number(..., sheet_id=...)`.
    """
...
def stamp_status(
    ...
    * No-op skip: an already-at-target row (same Status, and no new Sent At /
    ...
    if status not in LEGAL_STATUSES:
        raise ValueError(
            f"illegal PO_Log status {status!r} (legal: {sorted(LEGAL_STATUSES)})"
        )
    row = find_row_by_po_number(po_number)
    if row is None:
        return False
