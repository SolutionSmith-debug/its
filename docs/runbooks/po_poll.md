...
>   - Purchase order — build, config, pull/render/file, send (dark) → A PO is built + signed in the portal → “A PO is refused at pull with a totals mismatch.”
>   - Purchase order — build, config, pull/render/file, send (dark) → po-poll verifies, renders, files, and logs the PO → “POs are not being pulled/filed.”
>   - Purchase order — build, config, pull/render/file, send (dark) → po-poll verifies, renders, files, and logs the PO → “A PO attachment is refused (SUSPICIOUS/MALICIOUS) or the attachment pass errored.”
