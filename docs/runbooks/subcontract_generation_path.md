...
>   - Subcontract — build, legal gate, pull/render, send (dark) → A subcontract is built (with Exhibit A / SoV) in the portal → “A subcontract is refused at pull with an SoV mismatch or fails the legal gate.”
>   - Subcontract — build, legal gate, pull/render, send (dark) → subcontract-poll verifies and renders the package (three files) → “Subcontracts are not being pulled/rendered.”
