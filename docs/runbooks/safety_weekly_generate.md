...
>   - Safety report — portal submission to sent weekly packet → weekly-generate compiles the Sat→Fri packet (generation half) → “The Friday compile did not run (laptop asleep / crash) and no packet was produced.”
>   - Safety report — portal submission to sent weekly packet → weekly-generate compiles the Sat→Fri packet (generation half) → “One job's week failed to compile and was routed to the Review Queue; others compiled fine.”
> Daemon(s): `weekly-generate` — see the [daemon reference](../references/daemon_reference.md).
<!-- END TREE-XREF -->
