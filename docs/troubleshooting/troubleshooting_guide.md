...
- Only a FEW jobs named, or the SAME jobs every cycle? Half-or-more is a low bar on a small deployment — work the "a CRITICAL names ONE job" checks below first; a renamed folder never self-heals.
...
- A compile_now_scan_sustained CRITICAL alongside this one does NOT mean a broad outage — on a small deployment one broken job is already half the scanned jobs, so both rows fire for the SAME fault. This row is the specific one; work it first.
