...
- **S-1 (systemic) — MECHANISM DONE 2026-07-06 (#481 `c04f4cd`, four-part verify CLEAN):** the tracked `REQUIRED_CONFIG` startup-logging pass (#336) is BUILT — `shared/required_config.py` + `resolve_and_log` wired into ALL daemons; each declares a module-level `REQUIRED_CONFIG` list; a missing declared row now WARNs `config_row_missing` **distinctly** (no longer silent), each resolved setting logs its source (ITS_Config vs default); the §52 `narrated_controls` ledger entry `required_config_observable_resolution` flipped `dated_exception`→`enforced`. Residual (OPEN): the two named cross-workstream footgun rows still must be SEEDED correctly (unchanged); the shared `sheet_capacity` global keys (`sheet_count_ceiling`/`_margin`) are a documented carve-out (owned by that helper's own observability — a bounded follow-up, see the `required_config.py` docstring).

**Tag:** `smartsheet`, `daemon-health`, `config`, `capacity`, `audit`, `field_ops`.

## M3 Material Incidents — Slice 2/3 open items [OPEN 2026-07-06]

Full M3 (operator-chosen) is being built in 3 slices; Slice 1 (validated `line_uuid` on the incident submission) landed (#482 `eca4c64`). Remaining, with the decisions the fresh session must make (full grounded brief in auto-memory `project_fieldops-portal-program.md` + memory-archive §G57):

- **Slice 2 data-source decision (the one real fork):** the dedicated Material Incidents Smartsheet up-syncs from either (i) the `material-incident-v1` **form submissions** (carry the Slice-1 `line_uuid` + narrative/photos; needs a NEW `/api/internal` endpoint `SELECT … FROM submissions WHERE form_code LIKE 'material-incident%'` + a NEW `portal_client` method — **recommended**, uses Slice 1's foundation) or (ii) the **existing** `get_fieldops_material_list_snapshot` filtered to `status='incident'` (zero new surface, but active-lines-only, line-state only, and `line_uuid` goes unused). `material_incidents.py` is a write-HELPER cloning `material_list.py` — **no `GATED_SCRIPTS`/`WALKED_ROOTS`/picklist-`REGISTRY` edit** (Status is TEXT controlled-vocab, not a picklist); 5 clone-the-materials-pattern edits in `fieldops_sync.py`. Must clone the zero-drop `_reconcile_job_zeroed` guard (the #468 class — M3 is its next consumer) + a Check-O row-cap monitor + a mandatory live smoke.
- **Slice 3 RE-SCOPED — likely DROP:** incident photos are **already §34-screened inline** by `intake._screen_portal_photos` (the form's header `photo` field) on every submission — the security goal is MET. A "fenced `portal_poll` deep-screen pass" is real work ONLY if migrating incident photos onto the Option-D per-photo pool (an architecture/consistency choice, not a gap). Decide DROP vs Option-D with the operator.
- **`incidents_enabled` dark-gate SEED:** when Slice 2 lands, the ITS_Config row `field_ops.fieldops_sync.incidents_enabled=false` (Workstream=`field_ops`) MUST be created via MCP — there is no seed script, and `_read_bool_setting` treats an absent row identically to `false` (the recurring #468/#470 dark-gate-no-row reflex, HOUSE_REFLEXES §5).

**Tag:** `field_ops`, `progress_reports`, `section51`, `material-incidents`, `roadmap`.
...
## Cloudflare Worker deploy pending — PR #475's photo-required enforcement route not yet live [OPEN 2026-07-05]

**Checklist program (PR #475, merged `58a5267`, four-part verify CLEAN).** The Worker-side change in this PR — the item-state `source_item_id → checklist_items` join that surfaces `requires_photo` to the assignee, and the `/item-state/:id/complete` server-side re-enforcement (400 `photo_required` until a live `pending`|`clean` `item_photos` row exists) — is code-complete and tested (Worker vitest 805 passed, including the new `requires_photo` completion-enforcement + authoring/gating/round-trip tests) but **not deployed**. `npm run deploy` from `safety_portal/` has not run since before this PR merged. Until it runs: the checklist form-builder UI (SPA) is unaffected (SPA-only changes deploy independently via the same command), but the photo-required *enforcement* is dark — the client-side "Requires photo" gate exists, but the server-side re-check is not live, so a client that skips the UI gate is not currently blocked.

**Fix:** operator runs `npm run deploy` from `safety_portal/`. **Coupled with the Hours Log migration below** — the same deploy also completes PR #472's live-sheet migration sequence (`--phase add --commit` must run BEFORE this deploy; see the entry below), so do not deploy until that migration phase has run.

**Tag:** `field_ops`, `safety_portal`, `deploy`, `checklists`, `photo-required`.

## Hours Log — replace Started/Ended columns with a Task column [BUILT 2026-07-05 — live-sheet migration is now a LIVE-FIRE blocker, not just a pending step]

**UPDATE 2026-07-05 (checklist-program session, PR #475 window) — the KeyError this entry warned about is now LIVE, not hypothetical.** The `~/its` daemon tree had been stale at `f7f3764` (pre-#472) since PR #472 merged; this session pulled it forward to `58a5267` to deploy PR #475's Worker changes, which put PR #472's Task-column-writing code onto the live daemon for the first time. The live `Portal create test 2 — Hours Log` sheet still lacks the `Task` column, so `fieldops_sync`'s next hours-mirror cycle will hit `shared.smartsheet_client.add_rows`'s `KeyError` on the unknown column title exactly as this entry predicted — a non-self-healing Fault-M failure, not a silent skip. CC previewed `scripts/migrations/hours_log_task_column.py --phase add` in dry-run to confirm the live column really is absent, but correctly declined to run `--phase add --commit` against the live shared-SoR sheet (an operator-authorized schema write, not a CC-autonomous action). **Operator go-live sequence, order-critical, now urgent (the pulled tree is live):** (1) `--phase add --commit`, (2) `npm run deploy` (also activates PR #475's photo-required Worker route, separately pending), (3) `--phase drop --commit`. See memory-archive §G56.3 for the fuller writeup of the general "pulling a stale tree forward can activate a merged-but-undeployed migration dependency" lesson. Original BUILT entry below, unchanged.
