...
| `safety_reports.box.portal_root_folder_id` | str | *(unset)* | Shared Box mirror-tree root; owned by safety_reports. Clean estimates file under ROOT→<job>→'Purchase Orders'→'Vendor Quotes'. | po_materials.estimate_poll, po_materials.po_poll, safety_reports.intake, safety_reports.portal_poll, safety_reports.weekly_generate, subcontracts.subcontract_poll |
...
| `safety_reports.portal.worker_base_url` | str | *(unset)* | Shared Worker base URL; owned by safety_reports, read here too. | field_ops.fieldops_sync, po_materials.estimate_poll, po_materials.po_poll, safety_reports.portal_poll, safety_reports.publish_daemon, subcontracts.subcontract_poll, watchdog |
